...
// Estimation Metrics Carousel Component
const EstimationMetricsCarousel: React.FC<{ metrics: any; formatCurrency: (amount: number) => string }> = ({ metrics, formatCurrency }) => {
  ...
  const metricCards = [
    ...
    },
    {
      ...
      subtitle: `Cost: ${metrics.costRejections}, PM: ${metrics.pmFlagRejections}`
    },
    {
      title: 'TD Rejected',
      ...
      change: metrics.tdRejectedCount > 0 ? '+' + metrics.tdRejectedCount : '0',
    ...
    {
      title: 'Processing',
      value: `${metrics.avgProcessingTime || 0}d`,
      icon: BarChart3,
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % Math.max(1, metricCards.length - 5));
    }, 3000);
  ...
  const handlePrev = () => {
    if (intervalRef.current) clearInterval(intervalRef.current);
    setCurrentIndex((prev) => (prev - 1 + Math.max(1, metricCards.length - 5)) % Math.max(1, metricCards.length - 5));
    intervalRef.current = setInterval(() => {
      setCurrentIndex(prev => (prev + 1) % Math.max(1, metricCards.length - 5));
    }, 3000);
  };

  // Get visible cards (show 6 at a time)
  const visibleCards = metricCards.slice(currentIndex, currentIndex + 6);
  if (visibleCards.length < 6) {
    visibleCards.push(...metricCards.slice(0, 6 - visibleCards.length));
  }
    ...
    <div className="mb-6">
      <div className="relative">
        ...
        <div className="flex justify-center gap-1 mt-3">
          {Array.from({ length: Math.max(1, metricCards.length - 5) }).map((_, index) => (
            <button
              key={index}
              onClick={() => {
                if (intervalRef.current) clearInterval(intervalRef.current);
              ...
              className={`transition-all ${index === currentIndex
                ? 'w-6 h-1.5 bg-[#243d8a] rounded-full'
...
const EstimationHub: React.FC = () => {
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-6">
      ...
      {/* Metrics Carousel */}
      <EstimationMetricsCarousel metrics={metrics} formatCurrency={formatCurrency} />

      {/* Old Metrics Cards - Hidden */}
      <div className="hidden grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-3 mb-4">
