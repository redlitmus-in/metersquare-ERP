...
// Site Supervisor Metrics Carousel Component
const SiteSupervisorMetricsCarousel: React.FC<{ metrics: any; formatCurrency: (amount: number) => string }> = ({ metrics, formatCurrency }) => {
  ...
  const metricCards = [
    ...
    {
      title: 'Processing',
      value: Math.round(metrics.pendingCount * 0.3),
      icon: TrendingUp,
...
const SiteSupervisorHub: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen [&_*:focus]:outline-none [&_*:focus]:ring-0">
      ...
      {/* Metrics Carousel */}
      <SiteSupervisorMetricsCarousel metrics={metrics} formatCurrency={formatCurrency} />

      {/* Old metrics grid - hidden */}
      <div className="hidden grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
